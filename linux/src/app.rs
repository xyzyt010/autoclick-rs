...
fn start_sender(inner: &Rc<Inner>, ui: &AppWindow, id: i32) {
    let (key_indices, key_count, sec_s, min_s, duration_s, tgt_pos, window_id) = {
        let panels = inner.panels.borrow();
    ...
    // Compute interval from seconds + minutes.
    ...
    let sender = KeySender::start(inner.ds, window_id, keys, interval, duration);
...
fn poll_events(inner: &Rc<Inner>, ui: &AppWindow, rx: &crossbeam_channel::Receiver<(i32, Event)>) {
    ...
    // Only sync UI when there was an actual event (avoids needless redraws).
    if had_event {
        sync_panel(inner, ui);
    }
}
