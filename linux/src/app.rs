...
use crate::detect::{self, DisplayServer};
...
fn key_desc(idx: usize) -> String {
    ...
    if idx < keys.len() {
        let info = keys[idx].1;
        format!("keysym=0x{:04X}  code={}", info.keysym, info.keycode)
    } else {
        String::new()
    }
}
...
impl App {
    pub fn run() {
        ...
        // Leak timer so it lives forever.
...
fn refresh_targets(inner: &Rc<Inner>, ui: &AppWindow, id: i32) {
    let (mode, ds) = {
        ...
        match panels.iter().find(|p| p.id == id) {
            Some(p) => (p.mode, inner.ds),
            None => return,
        }
    };

    let my_pid = std::process::id();
    let targets = targets::enumerate(ds, mode, my_pid);
...
fn filter_targets(inner: &Rc<Inner>, ui: &AppWindow, id: i32, txt: &SharedString) {
    ...
    if let Some(p) = panels.iter_mut().find(|p| p.id == id) {
        ...
        if query.is_empty() {
            ...
            // Filter: items starting with query first, then contains.
...
fn start_sender(inner: &Rc<Inner>, ui: &AppWindow, id: i32) {
    let (key_index, interval_s, duration_s, tgt_idx, tgt_len, window_id) = {
        let panels = inner.panels.borrow();
        match panels.iter().find(|p| p.id == id) {
            Some(p) => {
    ...
    let sender = KeySender::start(inner.ds, window_id, key, interval, duration);
